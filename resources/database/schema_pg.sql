...
CREATE TABLE t_conversation_task (
    id                 VARCHAR(20)  NOT NULL PRIMARY KEY,
...
CREATE UNIQUE INDEX uk_conversation_active_task ON t_conversation_task (conversation_id, user_id) WHERE is_active = 1 AND deleted = 0;
CREATE INDEX idx_conversation_task_user_time ON t_conversation_task (conversation_id, user_id, last_active_time);
CREATE INDEX idx_conversation_task_topic ON t_conversation_task (conversation_id, user_id, topic_key);
CREATE INDEX idx_conversation_task_state ON t_conversation_task USING gin(state_json);
...
CREATE TABLE t_conversation_task_turn (
    id                   VARCHAR(20)  NOT NULL PRIMARY KEY,
    conversation_task_id VARCHAR(20)  NOT NULL,
    conversation_id      VARCHAR(20)  NOT NULL,
    user_id              VARCHAR(20)  NOT NULL,
...
CREATE INDEX idx_task_turn_conversation_task_time ON t_conversation_task_turn (conversation_task_id, create_time);
CREATE INDEX idx_task_turn_conversation_time ON t_conversation_task_turn (conversation_id, user_id, create_time);
CREATE INDEX idx_task_turn_state ON t_conversation_task_turn USING gin(turn_state_json);
...
CREATE TABLE t_conversation_retrieval_snapshot (
    id                       VARCHAR(20)  NOT NULL PRIMARY KEY,
    conversation_id          VARCHAR(20)  NOT NULL,
    user_id                  VARCHAR(20)  NOT NULL,
    conversation_task_id     VARCHAR(20)  NOT NULL,
...
CREATE INDEX idx_retrieval_snapshot_conversation_task_time ON t_conversation_retrieval_snapshot (conversation_task_id, create_time);
CREATE INDEX idx_retrieval_snapshot_turn ON t_conversation_retrieval_snapshot (task_turn_id);
CREATE INDEX idx_retrieval_snapshot_conversation_time ON t_conversation_retrieval_snapshot (conversation_id, user_id, create_time);
CREATE INDEX idx_retrieval_snapshot_refs ON t_conversation_retrieval_snapshot USING gin(result_refs_json);
