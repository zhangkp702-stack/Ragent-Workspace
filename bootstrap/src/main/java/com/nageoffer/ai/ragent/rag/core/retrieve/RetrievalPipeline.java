...
public class RetrievalPipeline {
    ...
    public RetrievalContext retrieve(List<SubQuestionIntent> subIntents, int topK) {
        ...
        List<CompletableFuture<SubQuestionContext>> tasks = subIntents.stream()
                .map(si -> CompletableFuture.supplyAsync(
                        () -> {
                            try {
                                // 对针对一个子问题，把它对应的 KB 知识库检索结果、MCP 工具调用结果都整理出来，封装成一个 SubQuestionContext
    ...
    // 针对一个子问题，把它对应的 KB 知识库检索结果、MCP 工具调用结果都整理出来，封装成一个 SubQuestionContext
    ...
    private String executeMcpAndMerge(String question, List<NodeScore> mcpIntents) {
        ...
        // 如果至少有一个 MCP 工具调用成功，就把工具返回结果和 MCP 意图一起交给 contextFormatter 格式化
    ...
    // 根据用户问题和命中的 MCP 意图节点，找到对应的 MCP 工具定义，提取工具调用参数，并构造一个可以执行的 MCPRequest
