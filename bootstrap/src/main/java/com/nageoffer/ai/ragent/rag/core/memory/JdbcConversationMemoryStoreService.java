...
@Slf4j
@Service
public class JdbcConversationMemoryStoreService implements ConversationMemoryStoreService {

    private final ConversationService conversationService;
    private final ConversationMessageService conversationMessageService;
    private final MemoryProperties memoryProperties;

    public JdbcConversationMemoryStoreService(ConversationService conversationService,
    ...
    public List<ChatMessage> loadHistory(String conversationId, String userId) {
        ...
        List<ChatMessage> result = dbMessages.stream()
                // 把数据库记录转换为 ChatMessage 消息
                .map(this::toChatMessage)
                // 过滤出历史记录消息
                // 转换失败的消息
                // role 不合法的消息
                // content 为空的消息
                // 不属于 user / assistant 的消息
